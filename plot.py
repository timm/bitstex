...
def generate_heatmap(file_path):
    ...
    heatmap_data = df_filtered.pivot_table(
        index='Y', columns='X', values='Value', aggfunc='median')
    ...
    x_labels = [int(col) if int(col) % 10 == 0 else '' for col in blurred_df.columns]
    y_labels = [int(idx) if int(idx) % 2 == 0 else '' for idx in blurred_df.index]
    ...
    plt.ylabel('Check', fontsize=12, fontweight='bold')
