...
\usepackage{array}              % for \arraybackslash, ragged in tabularx
\newcolumntype{Y}{>{\RaggedRight\arraybackslash}X}


\definecolor{codebg}{gray}{0.98}      % named: codebg
\definecolor{clsbg}{gray}{0.9}        % named: clsbg
\definecolor{Maroon}{RGB}{128,0,0}    % named: Maroon
\definecolor{darkyellow}{RGB}{218,165,32}  % named: darkyellow

\begin{document}

\title{Is Simple Also Stupid? Lessons from Building EZR,
       a 500-Line AI Toolkit}
...
\address[2]{\orgdiv{Department Name},
...
\abstract[Abstract]{
\renewcommand{\baselinestretch}{0.9}
EZR is a 500-line Python toolkit, with no heavy dependencies, that performs active learning, classification, multi-objective optimization, regression-tree explanation, and text mining. Despite this size, prior studies show EZR matches state-of-the-art explanation tools such as SHAP and LIME~\cite{amirali}, multi-objective optimizers including SMAC and TPE~\cite{kashin}, and the FASTREAD active learner for SLR relevance filtering~\cite{yu2018finding}. The cost reductions are large: 10× to 10{,}000× fewer lines, labels, and CPU cycles than those tools.
*We attribute these results to careful refactoring. Beneath the surface, the algorithms in EZR share one substrate of incrementally maintained \cls{Num}, \cls{Sym}, and \cls{Data} distributions. With that substrate in place, Naïve Bayes is two functions, 
k-means is thirteen lines, 
k-means++ is eleven, the (1+1)-EA is fifteen, and TWCNB is thirty. The textbook treats these as separate algorithms, but in code they are uses of the same machinery.*
We report this against a recent argument that humans need not read AI code. The paper is the rebuttal: the tool exists because we read it carefully. \texttt{pip install ezr} for the package; data are at \texttt{github.com/timm/moot}. The question we asked at the start—is simple also stupid?—receives, after testing across 120+ SE tasks, a clear no.}
...
\author{Menzies T.},
\author{Author Two}},
\ctitle{Is Simple Also Stupid? Lessons from Building EZR,
        a 500-Line AI Toolkit}.
...
Several voices in 2025 and 2026 argued that humans no longer need
to read code. Ryan Dahl, who created Node.js, suggested the era
of human-written code is ending. Jensen Huang advised against
learning to code. Elon Musk said traditional coding ends in 2026.
Replit's CEO told students to focus on how to talk to machines,
not how to program. The phrasing varied. The claim was the same:
AI is the new compiler, and you do not read what a compiler
emits.

We disagree. Reading code is useful. Refactoring code is more
useful. Things on the ground get missed when you fly over.

This paper is the evidence. We report on \IT{}, a Python toolkit
of about 500 lines for AI in software engineering. \IT{} grew
across several years of repeated reading and rewriting. We did
not start with its current shape. We arrived at it by sitting
with the code, noticing similarities across distant parts of the
system, and deleting what those similarities made redundant. The
result is a small, dependency-light tool. On 120-plus
...
from fewer than 100 labels. It produces decision trees that use
...
repository~\cite{menzies2025moot} used in this paper. Each row
is a task category. \#y is the number of goal columns; \#x is
the range of independent attribute counts; Rows is the row-count
range across the tasks in that category.}
\label{mootdata}
...
\subsection*{Five lessons}

This paper documents five lessons from building \IT{}.

\begin{itemize}
\item Many models can be tiny. Across 120-plus MOOT tasks,
fewer than ten variables suffice to model behavior, even when
thousands are available (\S\ref{sec:tiny}).
\item Random search works better than expected. Three classical
optimizers (simulated annealing, local search, the (1+1)-EA) are
statistically indistinguishable on MOOT. They differ only in a
one-line accept rule (\S\ref{sec:opt}).
\item One data structure suffices. Classification, clustering,
optimization, regression, and text mining all use the same
incremental \cls{Num}/\cls{Sym}/\cls{Data} substrate
(\S\ref{sec:peek}, \S\ref{sec:famous}, \S\ref{sec:active}).
\item Optimization, classification, and regression are nearly the
same algorithm. What looks in the textbook like five algorithms
is, in code, five uses of one substrate (\S\ref{sec:famous},
\S\ref{sec:opt}).
\item Small codebases work. Fewer dependencies mean less
supply-chain risk, easier review, faster porting, and more
visible structure for future refactoring.
\end{itemize}

\subsection*{The same objection, five times}
...
\includegraphics[width=\linewidth]{kit.pdf}
\caption{Recall (top row) and false alarm (bottom row) as a
function of labels acquired. Blue = non-normalized TWCNB; red =
normalized TWCNB. Warm-start = 25; each curve is the median
over 20 repeats with shaded IQR. Vertical bars mark the labeling
budget at which Yu et al.~\cite{yu2018finding} reached the same
recall with FASTREAD; numerical labels at left mark the recall
right after the warm start.}
\label{slrfig}
\end{figure*}
