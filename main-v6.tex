...
We disagree. Reading code is very useful. Important low-level information  ``on the ground'' (as it were)  gets missed when developers always use LLMs to  ``fly over'' the details.
...
The result was a small, very fast, dependency-light tool. 

  EZR has been tested 
on 120+
multi-objective tasks  shown in
Figure~\ref{mootdata} from the MOOT
repository~\cite{menzies2025moot} (these tasks are from recent papers in the software engineering literature
published in
ICSE, FSE, TSE,
...
As shown later in ths paper, for these tasks,
\IT{} builds effective models
from fewer than 100 labels. It produces effective regression  trees that use
fewer than ten variables, even when thousands were available. It
...
which we use to structure this paper
as follows:


\begin{enumerate}
\item In \S\ref{sec:peek}, \S\ref{sec:famous}, and
  \S\ref{sec:active} shows that {\em a handful of data structures
    suffices for a wide range of AI tools}. Classification,
clustering, optimization, regression, active learning, and text
mining all can use the same incremental
\cls{Num}/\cls{Sym}/\cls{Data} substrate.

\item In \S\ref{sec:famous} and \S\ref{sec:opt}, we show that
  {\em optimization, classification, and regression are nearly the same
    algorithm}. What looks in the textbook like separate fields is, in
code, repeated use of one substrate.

\item In \S\ref{sec:opt}, we show that {\em random search works better
  than might be expected}. Three classical optimizers (simulated annealing,
local search, (1+1)-EA) are statistically indistinguishable on
MOOT. 

\item In \S\ref{sec:active}, we show that
  {\em the same substrate
yields an active learner that runs 10{,}000$\times$ faster than
    the state-of-the-art} while matching its solution quality, using O(1) incremental
updates instead of tree-ensemble refits.
In prior work, \IT{} matched state-of-the-art
explanation tools (SHAP, LIME, Anchors)~\cite{Amiraliminimaldata} and
multi-objective optimizers (SMAC, TPE,
HyperOpt)~\cite{ganguly2026lowgodatalightse}, while using 10 to 10{,}000
times less code, less data, and less compute.

\item \S\ref{sec:tiny} and \S\ref{sec:valid}, shows that
  {\em many models can be tiny}. Across 120-plus MOOT tasks, fewer than
ten variables suffice even when thousands are available---and
held-out validation shows these small trees perform very well and
    can
   reach 85--95\% of the reference
optimum.

\item In \S\ref{sec:textmine}, we show that {\em 30 lines of TWCNB on
the same substrate matches state-of-the-art text mining
    for relevancy filtering},
reanalyzing Yu et al.~\cite{yu2018finding} on relevance filtering
for systematic literature reviews.
Thirty lines of TWCNB, riding on the same substrate,
matched Yu's results with roughly one-seventh of the labels
and exposed a measurement gap (false alarm) the original study
could not have found because it did not measure that metric.
\end{enumerate}
Overall, we show that {\em small codebases can work very well}.
Three independent comparisons, three domains, the same
pattern: simple, when carefully built, is not stupid.
EZR is 500 lines, runs in milliseconds, and has no heavy dependencies,
which means less supply-chain risk, easier review, faster
porting, and more visible structure for the next round of
refactoring.

Of the above, we have publshed (4) previsouly. The rest is new work for this paper.

\subsection{Caveat}

Before beginning, we digress to make an important caveat.

Not all AI tasks can be simplified this way. Generative tasks
including translation, summarization, and natural-language
interfaces do need LLMs and the data scaling that come with them.
So this paper is \underline{\bf not} saying that LLMs are unnecessary. 

What we do say is that many
SE tasks (configuration, tuning, defect prediction, testing,
cost estimation, relevance filtering) admit very simple
solutions. Hence we recommend a ``simplicity first'' approach:
try the simplest possible implementation before anything else.
Even when that first step is later outperformed, it establishes
a strict performance floor (so you can measure exactly what the
complex model adds), provides transparency a black-box model
cannot, and shifts the burden of proof onto the more expensive
alternative.
