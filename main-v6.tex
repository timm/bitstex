...
% ---------------- Fonts ----------------
% pdflatex: inconsolata loaded above. xelatex: load JetBrainsMono if present.
\ifPDFTeX\else
...
% ---------------- Listings setup (pdflatex-safe, no Pygments) ----------------
\captionsetup[lstlisting]{skip=0pt,position=top}
\lstset{
  language=Python,
  ...
  numbers=left,
  firstnumber=last,
  numberstyle=\color{gray}\fontsize{6}{6}\selectfont,
  numbersep=2mm,
  xleftmargin=0pt,
  breaklines=true,
...
\lstnewenvironment{repl}{\lstset{numbers=none}}{}

% Keep listings blocks from breaking across columns
...
\item
\S\ref{sec:famous} implements Na\"{\i}ve Bayes, $k$-means, and
...
algorithm asking slightly different questions. Na\"{\i}ve Bayes takes two
...
row score against them? Na\"{\i}ve Bayes scores by likelihood;
...
The core of Na\"{\i}ve Bayes is one question: how much does a column
...
Na\"{\i}ve Bayes (classify each row with the model so far, then add
...
\begin{itemize}
\item Na\"{\i}ve Bayes asks: how much does a column like a value?
...
``Na\"{\i}ve Bayes,'' ``$k$-means,'' and ``$k$-means++'' will
...
support vector machines. ~\cite{yu2018total,Yu2018}. This section shows
...
labeling budget at which Yu et al.~\cite{Yu2018} reached
...
Na\"{\i}ve Bayes, $k$-means, and $k$-means++ (\S\ref{sec:famous});
